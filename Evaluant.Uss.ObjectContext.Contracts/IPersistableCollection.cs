...
namespace Evaluant.Uss.ObjectContext.Contracts
{
...

    {
        ...
        public IPersistableCollection(IPersistenceEngineObjectContext oc, IPersistable parent, string role, IEnumerable<T> data)
            : this((IPersistenceEngineObjectContextAsync)oc, parent, role, data)
        {
            this.oc = oc;
        }
        ...
        internal void EnsureLoaded()
        {
            if (IsLoaded)
            {
                ...
                {
                    isLoaded = true;
                    MultipleEntry entities = (MultipleEntry)_Parent.Entity[_Role];
                    IList<T> items = ocAsync.LoadWithEntities<T>(entities);
                    foreach (T o in items)
                        _List.Add(o);
                }
            ...
            if ((ObjectContext != null && !ObjectContext.IsLazyLoadingEnabled) || (ObjectContextAsync != null && !ObjectContextAsync.IsLazyLoadingEnabled))
                return;

            if (oc != null && IsAttached)
            {
                Load(oc.LoadReference<T>(_Parent.Entity, _Role));
            }
        ...
        private void Load(IList<T> list)
        {
            _List = list;
            isLoaded = true;
            _Parent.Entity.InferredReferences.Add(_Role);
            _Parent.Entity.Add(multipleEntry);
            foreach (var item in list)
                ocAsync.Resolver.Resolve(item, ocAsync.Factory.PersistenceEngineFactory.Model);
        ...
        {
            Load(ocAsync.EndLoadReference<T>(result));
        }
        ...
        private Entity GetEntity(object item)
        {
            ...
            if (persistable != null)
                ...
                return oc.Resolver.Resolve(item, ocAsync.PersistenceEngineAsync.FactoryAsync.Model);
        }
