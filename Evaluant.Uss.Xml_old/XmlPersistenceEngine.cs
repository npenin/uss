...
namespace Evaluant.Uss.Xml
{
    ...
    public class XmlPersistenceEngine : PersistenceEngineImplementation
    {
        ...
        public XmlPersistenceEngine(string filename, Model model)
        {
            _Model = model;
            LoadFile(filename);
        }
        ...
        public override EntitySet Load(Evaluant.OPath.OPathQuery query, string[] attributes, string orderby, int first, int max)
        {
            if (first <= 0)
